...
molecule_3 = MyOwnDataset( root = '', molecule = 3)  -> will return the Data object for the SMILES 3
...
class MyOwnDataset(InMemoryDataset):
    ...
    def __init__(self, root,molecule, transform=None, pre_transform=None):
        ...
        self.data_3, self.slices_3  = torch.load(self.processed_paths[2])
        if molecule == 1:
            self.data , self.slices =self.data_1, self.slices_1
        elif molecule == 2: 
            self.data , self.slices =self.data_2, self.slices_2
        elif molecule == 3: 
            self.data , self.slices =self.data_3, self.slices_3    
        else:
    ...
    def process(self):
        ...
        for _, row in df.iterrows():
            smiles_1, smiles_2, smiles_3, ratio_1, ratio_2, ratio_3, log_CMC, T_norm = row['smiles_string_surfactant_1'], row['smiles_string_surfactant_2'], row['smiles_string_surfactant_3'], row['ratio_1'],  row['ratio_2'], row['ratio_3'], row['log_CMC'], row['T_norm']  # T_norm is the normalized temperature between [0,1]. On the GNN model, the normalized temperature is re-scaled between [0,10].
            ...
            if ratio_3 == 0:
                x_3, edge_attr_3, edge_index_3  = torch.zeros((1,33), dtype=torch.float),  \
                                                  torch.zeros((1,8), dtype=torch.float),  \
                                                  torch.zeros((2,1), dtype=torch.int64)   
                smiles_3 = 'no_molecule' 
                h_donors_3, h_acceptors_3 = 'no_molecule', 'no_molecule' 
                h_inter = min(h_donors_1, h_acceptors_1) 
                h_inter_1_3 = 0
                h_inter_2_3 = 0
            else:
                x_3 = self.get_node_features(smiles_3)
                edge_attr_3, edge_index_3 = self.get_edge_features(smiles_3)  
                h_donors_3, h_acceptors_3 = self.get_hydrogen_bond(smiles_3)
                h_inter_1_3 = min(h_donors_1, h_acceptors_1) + min(h_donors_3, h_acceptors_3)
                h_inter_2_3 = min(h_donors_2, h_acceptors_2) + min(h_donors_3, h_acceptors_3)            
            ...
            data_3 = Data(x=x_3, 
                        ...
                        edge_attr=edge_attr_3,
                        T = T_norm, 
                        smiles_id_3 = smiles_3,
                        ratio_3 = ratio_3,
                        h_inter_1_3 = h_inter_1_3,
                        h_inter_2_3 = h_inter_2_3,
                        y=torch.tensor(target, dtype=torch.float))
        ...
        torch.save(self.collate(data_list_3), self.processed_paths[2])
