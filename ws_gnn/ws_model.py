...
@author: BrozosCh
"""
from ws_smiles_to_molecules import MyOwnDataset
import torch
...
parser.add_argument('--surfactant', default  = 'dodecyl_maltoside') # Surfactant to be excluded in split type 3 (sub-test set), referred to as mix-surf-extra in the manuscript
...
parser.add_argument('--lrpatience', default=2)   # number of consecutive epochs without model improvement after which learning rate is decreased
...
class GNNReg(torch.nn.Module):
    def __init__(self):
        ...
        self.transformation_layer = torch.nn.Linear(dataset_molecule_1.num_edge_features, dim)  # Initial linear transformation layer for edge features.
    ...
    def forward(self, data_molecule_1, data_molecule_2):
      ...
      # Message passing and global sum pooling for molecule 1.
      ...
      # Message passing and global sum pooling for molecule 2.
      ...
      # Weighted linear summation of the two molecular fingerprints. 
        fp = (x_1 * ratio_1.unsqueeze(1)) + (x_2 *ratio_2.unsqueeze(1))
...
# Load the task's corresponding dataset, i.e., split_type_1: comp-inter, split_type_2: mix-comp-extra, split_type_3: mix-surf-extra, split_type_4: mix-extra
...
def data_preparation(seed, dataset_molecule_1_input = dataset_molecule_1, dataset_molecule_2_input = dataset_molecule_2):
    torch.manual_seed(seed)
    dataset_molecule_1_input, perm = dataset_molecule_1_input.shuffle(return_perm= True)
    dataset_molecule_2_input = dataset_molecule_2_input[perm]
    val_dataset_1, val_dataset_2 = dataset_molecule_1_input[:385], dataset_molecule_2_input[:385]
    train_dataset_1, train_dataset_2 = dataset_molecule_1_input[385:], dataset_molecule_2_input[385:]
...
#  Function for returning the unstandardized predictions. Based on the seed, we can initiate it with different models.
...
# In this section, we define the number of runs we wish (20 during our work) and append the parameters to corresponding lists. Each run has a different training validation split.
